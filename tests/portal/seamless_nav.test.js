/* ============================================================================
   ...
   blank flash + make sidebar/tab clicks read as instant:
     ...
     PART A  pre-paint snippet — every shell page's <head> sets an inline
             ...
             styles/responsive.css (swiss); identical snippet on all 8 pages.
     ...
     PART C  speculation rules — every page carries a JSON-valid prerender
             document rule (moderate eagerness) with the side-effect exclusions
             (/device*, /oauth2/*, /api/*, /assets/*).
   ...
   a vm sandbox over a tiny fake DOM (mirrors project_switcher.test.js).
...
const PAGES = [
  ...
  "metrics.html", "settings.html", "projects.html", "onboarding.html",
];
const heads = {};
...
const expect = {
  ...
  swissDark: { bg: tokenIn(blockOf(responsiveCss, 'html\\[data-skin="swiss"\\],'), "bg"),
               fg: tokenIn(blockOf(responsiveCss, 'html\\[data-skin="swiss"\\],'), "text") },
  swissLight: { bg: tokenIn(blockOf(responsiveCss, 'html\\[data-skin="swiss"\\]\\[data-theme="light"\\]'), "bg"),
                fg: tokenIn(blockOf(responsiveCss, 'html\\[data-skin="swiss"\\]\\[data-theme="light"\\]'), "text") },
};
assert(expect.classicDark.bg && expect.classicLight.bg && expect.swissDark.bg && expect.swissLight.bg,
  "parsed all four --bg values out of tokens.css/responsive.css");
...
for (const p of PAGES) {
  ...
  for (const excl of ["/device*", "/oauth2/*", "/api/*", "/assets/*"]) {
...
// E4: cache key is per (cid, page) — another project's cache never leaks in
{
  const side = makeNode("sidebar"), top = makeNode("topbar");
  makeSandbox({
    store: { "orcha:shellHtml:OTHER:tasks": JSON.stringify({ side: "wrong-project", top: "wrong-project" }) },
    ids: { sidebar: side, topbar: top },
  });
  assert(side.innerHTML === "", "a different project's cached chrome is not restored");
}
