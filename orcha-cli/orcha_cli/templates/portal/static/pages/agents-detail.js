    ...
    <div class="card">
      ...
      <div class="card-b" style="padding:13px 14px;display:flex;flex-direction:column;gap:10px">
        ...
        <div class="ctrl"><div class="grow"><div class="lbl">Autonomy</div><div class="desc">${autOvrDesc(a)}</div></div>
          <div class="seg" id="autOvrSeg" data-agent="${AgeO.esc(a.id)}" aria-label="Per-agent autonomy override">${AUT_OVERRIDES.map((o)=>`<button type="button" class="${(a.autonomy_override||null)===o.id?'on':''}" data-ovr="${o.id==null?'null':AgeO.esc(o.id)}" aria-pressed="${(a.autonomy_override||null)===o.id?'true':'false'}" ${canEditAutOvr() ? "" : "disabled"}>${AgeO.esc(o.name)}</button>`).join("")}</div></div>`}
      </div>
    </div>
  </div>`;
...
// Cloud access model (mig 039): the selector is enabled only for owners / manage_autonomy
// holders — the SAME grant the container slider requires (the server enforces regardless;
// this only gates the AFFORDANCE). Viewers (role-viewer members AND trusted non-members)
// stay read-only: they still SEE the current override + effective level, chips disabled.
// An enforced container parks the control for everyone (the override is ignored server-side).
function canEditAutOvr() {
  if (!AgeO.actingHuman() || containerEnforced()) return false;
  if (typeof AgeO.actingReadOnly === "function" && AgeO.actingReadOnly()) return false;
  return typeof AgeO.actingGrant === "function" ? AgeO.actingGrant("manage_autonomy") : true;
}
function effectiveAutonomyOf(a) {
  // Prefer the server-computed field (single shared rule); degrade to the same rule computed
  // client-side for a pre-043 snapshot that omits it.
