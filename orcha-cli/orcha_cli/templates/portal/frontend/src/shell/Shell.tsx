...
function AutonomySwitch() {
  ...
  const setWakes = async (enabled: boolean) => {
    ...
    try {
      ...
      toast(res.wakes_enabled ? "Autonomy · Running" : "Autonomy · Paused", res.wakes_enabled ? "ok" : "");
      void refresh();
    } catch (e) {
  ...
  const click = (rung: number) => {
    if (!canAct) { toast("Pick an acting human to change autonomy", "warn"); return; }
    if (rung === 0) {
      setPending(paused
        ? { title: "Resume autonomy?", desc: "Agents resume waking at the current autonomy level. Restores the global wake switch to ON (Running).", primary: "Resume", run: () => void setWakes(true) }
        : { title: "Pause autonomy?", desc: "All agents stop waking immediately. In-flight work finishes; nothing new starts. Humans & live terminals still work.", primary: "Pause all wakes", danger: true, run: () => void setWakes(false) });
      return;
    }
    const rg = AUT_RUNGS[rung];
    if (!rg || !("level" in rg) || !rg.level || rg.level === level) return;
    setPending({
  ...
  return (
    <>
      <div className="aut-wrap" id="autWrap">
        <span className="aut-lab">autonomy</span>
        <div className={"aut" + (canAct ? "" : " locked")} role="radiogroup" aria-label="Container autonomy">
          {AUT_RUNGS.map((rg) => {
            if (rg.r === 0) {
              ...
              return (
                <span key={rg.r} className={cls} role="radio" aria-checked="true" title={tip} onClick={() => click(0)}>
            ...
            const tip = canAct
              ...
              : "Pick an acting human to change autonomy";
            return (
              <span key={rg.r} className={cls} role="radio" aria-checked={active} title={tip} onClick={() => click(rg.r)}>
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <div className={"pausebar" + (paused ? " show" : "")} id="pausebar">
          <span>⏸ Autonomy paused — no agent wakes. Humans &amp; live terminals still work.</span>
        </div>
