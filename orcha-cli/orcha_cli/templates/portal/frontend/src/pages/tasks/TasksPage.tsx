...
/* ============================================================================
   ...
   ========================================================================== */
function ThreadCard({ t, msgs, loading }: { t: Task; msgs: ThreadMsg[]; loading: boolean }) {
  const { snap, refresh } = useSnapshot();
  const toast = useToast();
...
export function TasksPage() {
  ...
  useEffect(() => {
    if (!t || !snap) return;
    const want = t.message_summary?.count || 0;
    const have = (threadsRef.current[t.id] || []).length;
    if (threadLoadingRef.current[t.id] || have >= want) return;
    threadLoadingRef.current[t.id] = true;
    threadOf(t.id, snap.agents)
      .then((th) => {
        threadsRef.current[t.id] = th;
        threadLoadingRef.current[t.id] = false;
        setThreadTick((n) => n + 1);
      })
      .catch(() => {
        threadLoadingRef.current[t.id] = false;
