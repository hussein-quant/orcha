...
export function classifyError(status: number, body: { reason?: string; detail?: string } | null): GhError {
  ...
  if (body && body.reason === "local_source") return { kind: "local_source", status, detail: (body && body.detail) || null };
...
export function classifyDetailError(status: number, body: { reason?: string; detail?: string } | null): GhError {
  ...
  if (reason === "local_source") return { kind: "local_source", status, detail: (body && body.detail) || null };
