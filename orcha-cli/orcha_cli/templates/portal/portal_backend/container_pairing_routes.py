...
def get_container_pairing(
    ...
    with db_cursor() as (_, cur):
        ...
        # human, and under proxy trust that human IS the verified member. A phone
        # is paired for yourself, never as someone else:
        #   - no explicit human requested → the resolved member is auto-selected
        #     (the multi-human 400 choose_human detour never fires for a member);
        #   - a mismatched explicit human → 403 outright, not a silent override —
        #     the UI dropped its "pair as" selector, so a foreign id here is a
        #     forged/stale request, not a UX path;
        #   - a trusted NON-member of a mapped container → 403 (trusted_actor).
        # write=False: pairing is read-scoped under the access model — the viewer
        # role may pair a phone to look around; every write still 403s there.
        resolved = trusted_actor(cur, request, cid, None, write=False)
        if resolved is not None:
            if human_agent_id is not None and human_agent_id != resolved:
                raise HTTPException(
                    403,
                    "pairing is identity-bound: you can only pair a phone as "
                    "your own signed-in member identity",
                )
            human_agent_id = resolved
        cur.execute(
